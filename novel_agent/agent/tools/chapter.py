...
def _resolve_chapter_target(novel_state, chapter_num: int = 0) -> tuple[int, str]:
    """
    解析续写目标章节
    当用户指定了 chapter_num (>0) 时，优先使用用户指定的章节号；
    否则自动查找大纲中下一个未写章节。
    """
    if chapter_num > 0:
...
def _update_outline_after_write(novel_state, chapter_num: int, title: str, full_content: str):
    """
    章节写入后更新大纲结构和元信息
    1. 计算章节内容的 SHA-256 hash 并存入 ChapterOutline.content_hash
    2. 更新或创建 ChapterOutline 条目（标题、摘要、字数、状态）
    3. 更新 meta.total_chapters
    4. 持久化 meta.json 和 outline_structure.json
    """
    ...
    if ch_in_outline:
        ...
        ch_in_outline.content_summary = full_content[: tc.chapter_content_summary_chars]
        ...
        novel_state.outline.chapters.append(
            ChapterOutline(
                title=title,
                content_summary=full_content[: tc.chapter_content_summary_chars],
                is_written=True,
                idx=chapter_num,
...
async def _stream_chapter_content(
    ...
    """
    流式生成章节内容并推送事件
    如果章节已有内容，LLM 只输出续写部分，本函数负责拼接已有内容。
    Returns:
        (完整内容, 结果消息)
    """
    ...
    return (
        full_content,
        f"第{chapter_num}章「{title}」已生成，请在编辑器中审核后保存，共{len(full_content)}字",
    )
...
async def _self_review_chapter(
    ...
    自检产生的修订内容通过 token 事件输出（聊天面板），不会覆盖编辑器状态。
    Returns:
        (内容, 是否需要重修)
    """
    ...
    w({"type": "token", "token": f"🔍 自检发现问题（{issue_desc[:30]}），正在自动修正...\n"})
    rewrite_prompt = (
    ...
    if revised.strip():
        ...
        w({"type": "token", "token": f"✅ 自修完成，修正了：{issue_desc[:50]}\n"})
        return revised, True
...
async def handle_continue_writing(
    ...
    if existing_ch and existing_ch.is_written:
        w(
            {
                "type": "token",
                "token": f"⚠️ 第{next_idx}章「{existing_ch.title}」已存在且已写入。如需重写，请使用 regenerate_chapter(chapter_num={next_idx})。\n",
            }
        )
        return f"第{next_idx}章已存在，跳过生成。如需重写请使用 regenerate_chapter。"

    if existing_title:
        title = existing_title
        w({"type": "token", "token": f"📌 使用大纲中的标题：{title}\n\n"})
    else:
        w({"type": "token", "token": f"📝 正在生成第{next_idx}章标题...\n"})
        title = await generate_chapter_title(novel_state, next_idx, user_request)
        w({"type": "token", "token": f"📌 标题：{title}\n\n"})

    w({"type": "chapter_title", "title": title, "chapter_num": next_idx})
    w({"type": "token", "token": f"✍️ 正在生成第{next_idx}章内容...\n"})
    ...
    NovelMemory.save_chapter(novel_state, next_idx, full_content)
    _update_outline_after_write(novel_state, next_idx, title, full_content)
...
async def handle_regenerate_chapter(
    ...
    if chapter_num <= 0:
        msg = "请指定要重新生成的章节号，例如「重新生成第九章」"
        w({"type": "token", "token": f"⚠️ {msg}"})
        return f"regenerate_chapter 失败：{msg}"

    chapter = novel_state.find_chapter_in_outline(chapter_num)
    if not chapter:
        msg = f"第{chapter_num}章不存在"
        w({"type": "token", "token": f"⚠️ {msg}"})
        return f"regenerate_chapter 失败：{msg}"
    ...
    if is_partial_rewrite:
        w(
            {
                "type": "token",
                "token": f"✏️ 正在局部重写第{chapter_num}章「{chapter_title}」...\n",
            }
        )
    else:
        w(
            {
                "type": "token",
                "token": f"🔄 正在重新生成第{chapter_num}章「{chapter_title}」...\n",
            }
        )
    ...
    if not is_partial_rewrite:
        full_content, _ = await _self_review_chapter(
            w,
            novel_state,
            chapter_num,
            chapter_title,
    ...
    _update_outline_after_write(novel_state, chapter_num, chapter_title, full_content)
    ...
    mode = "局部重写" if is_partial_rewrite else "重新生成"
    return (
        f"第{chapter_num}章「{chapter_title}」已{mode}并保存，共{len(full_content)}字"
    )
